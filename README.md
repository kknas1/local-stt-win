...
* 주로 제가 맥에서 개발을 하고 윈도우로 배포하고 있어서 윈도우에서 실행시 예상하지 못한 에러가 발생할수 있습니다. 
...
실행 후 화면에서 `파일 선택` 버튼으로 녹취 파일을 업로드하고, 오른쪽 위의 `처리 시작` 버튼을 누르면 변환이 시작됩니다. 변환이 끝나면 아래 화면처럼 로그, 요약, 전사 결과, 구간별 화자 정보가 표시됩니다.

![Local STT 실행 화면](docs/screenshots/local-stt-result.png)
